...
for IP in "${IP_ADDRESSES[@]}"; do
  ...
  for FILE in "${SOURCE_FILES[@]}"; do
    /usr/bin/expect <<EOF
    log_user 1
    spawn ssh -o StrictHostKeyChecking=no $USERNAME@$IP "[ -f $DEST_DIR/$FILE ] && echo 'File exists' || scp -o StrictHostKeyChecking=no /root/ceremonyclient/node/$FILE $USERNAME@$IP:$DEST_DIR"
    expect {
